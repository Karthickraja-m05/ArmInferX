...
class DeploymentCreateRequest(BaseModel):
    name: str = Field(..., description="Deployment release name", json_schema_extra={"example": "qwen2.5-prod-v1"})
    model_version_id: str | UUID = Field(..., description="Target model version ID or key", json_schema_extra={"example": "qwen2.5-0.5b-instruct"})
    configuration: dict[str, Any] = Field(
        default_factory=dict,
        description="Runtime parameter configuration (threads, batch_size, context_length, temp, etc.)",
    )
    environment: str = Field(default="production", description="Deployment environment (production, staging, development)")
...
class DeploymentResponse(BaseModel):
    ...
    status: str = Field(..., description="Deployment state (PENDING, STAGING, VERIFYING, ACTIVE, ROLLED_BACK, FAILED)")
    ...
    deployment_version: str = Field(..., description="Semantic deployment release version (e.g. v1.0.0)")
    ...
    health_status: str = Field(..., description="Health status (HEALTHY, DEGRADED, UNHEALTHY, UNKNOWN)")
    metrics_summary: dict[str, Any] = Field(default_factory=dict, description="Latest runtime metrics summary")
    created_at: datetime = Field(..., description="Creation timestamp")
    updated_at: datetime = Field(..., description="Last update timestamp")


class DeploymentRollbackRequest(BaseModel):
    reason: str | None = Field(default="Manual rollback triggered by operator.", description="Rollback justification")


class DeploymentRollbackResponse(BaseModel):
    success: bool = Field(..., description="True if rollback completed cleanly")
    restored_deployment_id: UUID | str = Field(..., description="ID of restored previous working deployment")
    rolled_back_deployment_id: UUID | str = Field(..., description="ID of rolled-back failing deployment")
    message: str = Field(..., description="Rollback outcome message")
    timestamp: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="Timestamp of rollback event")


class StageVerificationDetail(BaseModel):
    stage: str = Field(..., description="Stage name (startup, model_loading, inference, endpoint, resource)")
...
class DeploymentHealthCheckResponse(BaseModel):
    ...
    model_check: StageVerificationDetail = Field(..., description="Model loading verification detail")
    inference_check: StageVerificationDetail = Field(..., description="Inference token generation check detail")
    endpoint_check: StageVerificationDetail = Field(..., description="Endpoint probe detail")
    resource_check: StageVerificationDetail = Field(..., description="CPU and RAM resource check detail")
    timestamp: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="Health check timestamp UTC")


class DeploymentAlert(BaseModel):
    alert_id: str = Field(..., description="Unique alert ID")
    code: str = Field(..., description="Alert code (HIGH_LATENCY, HIGH_MEMORY, RUNTIME_FAILURE, ENDPOINT_FAILURE)")
...
class DeploymentMonitoringResponse(BaseModel):
    ...
    active_alerts: list[DeploymentAlert] = Field(default_factory=list, description="Currently active monitoring alerts")
    timestamp: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="Snapshot timestamp UTC")
...
class ConfigComparisonResponse(BaseModel):
    ...
    differences: list[ConfigDiffItem] = Field(default_factory=list, description="List of parameter differences")
