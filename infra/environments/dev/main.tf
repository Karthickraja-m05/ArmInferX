...
# Dynamic Account and Region Data Discovery (No Hardcoded Account IDs)
...
module "networking" {
  ...
  database_subnet_cidrs     = ["10.10.100.0/24", "10.10.200.0/24"]
...
# 3. Storage Module (S3 Bucket & KMS Key)
module "storage" {
  source = "../../modules/storage"

  project_name    = var.project_name
  environment     = var.environment
  force_destroy   = true # Enable force destroy for easy teardown in dev environment
  expiration_days = 30
  tags            = var.tags
}
...
# 6. Graviton ARM64 Compute Module
...
# 8. Deployment Infrastructure Module
module "deployment" {
